...
Kp = 0.0024  # 0.003125 kp=1/240，其中240是最大误差像素，1是希望最大误差时角速度是 ±1.0 弧度/秒
Kd = 0.0022
previous_error = 0
stop_flag = 0
boundary_flag = 0
...
def mid(follow, mask):
    ...
    for y in range(follow.shape[0] - 1, -1, -1):
        ...
        # 检查右半部分是否全为0
        if (mask[y][half:min(follow.shape[1], half + halfWidth)] == numpy.zeros_like(
                mask[y][half:min(follow.shape[1], half + halfWidth)])).all():
            right = min(follow.shape[1], half + halfWidth)  # 如果是，则右侧边界为当前右边界

        else:
            right = numpy.average(
                numpy.where(mask[y][half:follow.shape[1]] == 255)) + half  # 否则，计算右侧道路标记的平均位置，并加上中线位置（half）得到绝对位置

        left_white_count = numpy.sum(mask[y, :half] == 255)
        right_white_count = numpy.sum(mask[y, half:] == 255)
        if y == 245:
            for x in range(mask.shape[1]):  # mask.shape[1] is the number of columns
                if mask[y, x] == 255 and WhiteCount <= 100:  # mask[y, x] accesses the pixel value at (y, x)
                    WhiteCount += 1
            if WhiteCount >= 100:
                number += 1
        if number == 1:
            boundary_flag = 1
        if number == 2 and not boundary_flag:
            stop_flag = 1
        if y == 200:
            if boundary_flag == 2 or boundary_flag == 1:
                if left >= 10 or right <= 630:
                    boundary_flag = 0
                    # print("find straight road")
...
def xun_callback(message):
    ...
    while message.data == 1:
        ...
        if boundary_flag == 1:
            twist.linear.x = 0.12
            twist.angular.z = 0.9
            print("boundary_flag =", boundary_flag)
