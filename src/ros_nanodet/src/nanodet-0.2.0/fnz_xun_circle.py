    ...
    for y in range(follow.shape[0] - 1, -1, -1):
        ...
        if y == 245:
            ...
            for x in range(mask.shape[1]):  # mask.shape[1] is the number of columns
                if mask[y, x] == 255:  # mask[y, x] accesses the pixel value at (y, x)
                    WhiteCount += 1
            if WhiteCount >= 240:
                if right != 640 and left != 0 and not boundary_flag:
                    stop_flag = 1
                elif left != 0:
                    boundary_flag = 1
                elif right != 640:
                    boundary_flag = 2
                # print(f"boundary_flag = {boundary_flag}")
            if y == 180:
                if boundary_flag == 2 or boundary_flag == 1:
                    if left >= 20 and right <= 620:
                        boundary_flag = 0
