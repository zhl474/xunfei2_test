...
string output_file = "/home/ucar/ucar_car/src/line_follow/image/left2.avi";//录制视频避免网络传输卡顿
VideoWriter out;
...
void find_lefttrack_edge(Mat& gray_img, Point& left_point, int scan_rows, int brightness_threshold,Mat visualizeImg) {
        ...
        if (left_point.x == -1) {
            for (int x = middle_x - 1; x > 2; x--) {
                if ((int)gray_img.at<uchar>(y, x)>=brightness_threshold){ 
                    ...
                    circle(visualizeImg, left_point, 7, Scalar(0, 0, 0), -1);
                    break;
                }
            }
        }
...
void trace_leftedge(Point start_point, Mat& gray_img, vector<Point>& traced_points, bool& left, 
...
Point find_other_coner_edge(Mat gray_img,Point left_edge_point,int brightness_threshold,Mat& visualizeImg){//拐角的特征是这一行有下一行没
    ...
    if(left_edge_point.x>360){
        Point first_point = Point(700,700);
        bool flag1 = false,flag2 = false;
        for(int x=540;x>40;x--){//先把第一个点找出来
        ...
        if (!flag1) {
            ...
            // waitKey(1);
            return Point(700, 700);//第一个点都没找到就是还没有
        }
        ...
        for (int y = 52; y <= 180; y++) {
            bool flag = false;
            for (int x = maybe_point.x-80; x < 639; x++) {
                if (gray_img.at<uchar>(y, x) >= brightness_threshold) {
            ...
            if (!flag) {
                finded_count++;
                if(finded_count>4){
                    if(maybe_point.x<120) {
                        // imshow("test",visualizeImg);
                        // waitKey(1);
                        return Point(700,700);//才刚刚看到角点,不接受点在左边
                    }
        ...
        for (int y = 230; y > 55; y--) {
            bool flag = false;
            // ROS_INFO("扫描行%d",y);
            for (int x = 90; x < 490; x++) {
                if (gray_img.at<uchar>(y, x) >= brightness_threshold) {
                    // ROS_INFO("有超过阈值的点%d,%d",x,y);
                    int down_count = 0;//下两行不能有超过5个白点
                    circle(visualizeImg,Point(x,y), 5, Scalar(148, 66, 255), -1);
                    for(int i=x-10;i<x+10;i++){
                        if(gray_img.at<uchar>(y+1, i) >= brightness_threshold) down_count++;
                        if(gray_img.at<uchar>(y+2, i) >= brightness_threshold) down_count++;
                        if (down_count>5) {
                            // ROS_INFO("但是不满足条件被删掉了");
                            break;
                        }
                    }
                    int up_count = 0;//上两行必须有白点
                    for(int i=x-10;i<x+10;i++){
                        if(gray_img.at<uchar>(y-1, i) >= brightness_threshold) {
                            up_count++;
                            break;
                        }
                    }
                    for(int i=x-10;i<x+10;i++){
                        if(gray_img.at<uchar>(y-2, i) >= brightness_threshold) {
                            up_count++;
                            break;
                        }
                    }
                    if (up_count!=2) {  
                        circle(visualizeImg,Point(x,y), 5, Scalar(0, 0, 0), -1);
                        ROS_INFO("但是不满足条件被删掉了");
                        break;
                    }
                    if(down_count<5 && up_count==2){
                        maybe_point=Point(x, y);
                        circle(visualizeImg, maybe_point, 9, Scalar(0, 0, 255), -1);
                        flag = true;
                    }
            ...
            if (flag) {
                // imshow("test",visualizeImg);
                // waitKey(1);
                // ROS_INFO("找到点%d,%d",maybe_point.x,maybe_point.y);
                return maybe_point;
            }
        }
        // ROS_INFO("没找到点");
...
bool left_to_right(Mat gray_img,int brightness_threshold,bool& right_ready){//右巡线向左巡线跳变
    int height = gray_img.rows;
    int width = gray_img.cols;
    
    if(right_ready){
        int right_count = 0;//左边出现断线，即将进入圆环，圆环较近时进入左巡线
        for (int y = 279; y >= 200; y--) {
            for (int x = 320; x < 639; x++) {
                if (gray_img.at<uchar>(y, x) >= brightness_threshold) {
                    right_count++;
                    break;
                }
        ...
        int right_count = 0; //左边尚未断线，等待进入圆环
...
bool line_server_callback(line_follow::line_follow::Request& req,line_follow::line_follow::Response& resp){
    ...
    while(ros::ok()){
        ...
        if(!out_range && !pass_out && !other_enter && !pass_enter && !out_ready && !pass_enter_ready){//没有进入左逻辑才要判断需不需要右转左,只有第一圈需要判断
            // ROS_INFO("进入if");
            pass_out = left_to_right(gray_img,brightness_threshold,left_ready);
            if(pass_out) {
                ROS_INFO("第一次即将抵达出口%f",pose.response.pose_at[1]);
            }
        }
        ...
        if(!out_range && !pass_out && other_enter && !pass_enter && !out_ready){
            ...
            if(other_enter_last_conner.x != 700){
                ...
                out.write(cropped);
                if((abs(twist.linear.x)<0.08 && abs(twist.angular.z)<0.12)||pose.response.pose_at[2]<0){
                    other_enter = false;
                    pass_enter = true;
                    ROS_INFO("离开另一个路口");
                }
        ...
        int last_scanned_y;
        find_lefttrack_edge(gray_img,left_edge_point, scan_rows, brightness_threshold,cropped);
        // ROS_INFO("坐标跳变%d,定位%f",first_point_x_last - left_edge_point.x,pose.response.pose_at[0]);
        
        if ((out_range || pass_out) && left && (left_edge_point.x-first_point_x_last>220) &&pose.response.pose_at[0]<4.5){//如果右边线丢了或者右边界首个点发生剧烈左移动
            left = false;
            ROS_INFO("左巡线变为找右点%d,%d",first_point_x_last,left_edge_point.x);
        }
        else if(!left && (first_point_x_last-left_edge_point.x>220||left_edge_point.x<140 )){//左线发生剧烈偏移说明又看到右线了左跳右的幅度一般很剧烈|| (right_edge_point.y>170&&right_edge_point.x>300)
            left = true;
            ROS_INFO("重新开始左巡线:%d,%d",left_edge_point.x,first_point_x_last);
            if(pass_out){
                pass_out = false;
        ...
        if (left) {
            double line_error = 0;
            if(!double_line){
                trace_leftedge(left_edge_point, gray_img, traced_left, left_checker, brightness_threshold, &cropped);
                line_error = left_error_calculater(traced_left,left_edge_point.y,cropped);//有调试图片输出
            }
            else{
            ...
            if(!left_checker){
                // ROS_INFO("右线斜率出错，舍弃");
                displayStream <<"point: "<< traced_left.size();
                string displayText = displayStream.str();
                putText(cropped, displayText, Point(50, 50),FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 0), 1);
                ...
                if (out_range && !right_forward && !point_forward && (ros::Time::now()-start_time).toSec()>1.0){
                    double_line = true;
                ...
                displayStream << "error: " << line_error << "p:" <<line_error*p<< "i:" << integration*i<< "d: " << diff*d;
            ...
            if(right_forward){
                if(point_forward){
                    ...
                    if(pass_out){
                        ...
                        // ROS_INFO("转折点坐标x%d,y%d",right_edge_point.x,right_edge_point.y);
                        twist.linear.x = (205-right_edge_point.y)*other_enter_pointx;
                        twist.angular.z = (87-right_edge_point.x)*other_enter_pointy;
                        if(right_edge_point.x<190 && right_edge_point.y >180){
                            left = true;//恢复左巡线逻辑
                            other_enter = true;   //已经绕圆环半圈准备离开
                            pass_out = false;
                            ROS_INFO("到达另一个入口");//
                        }
                        displayStream <<"x:"<< twist.linear.x<<"z:"<< twist.angular.z<<"erx:"<<205-right_edge_point.y<<"ery:"<<553-right_edge_point.x;
                        string displayText = displayStream.str();
                        putText(cropped, displayText, Point(50, 50),FONT_HERSHEY_SIMPLEX, 0.5, Scalar(0, 0, 0), 1);
                        out.write(cropped);
                    }
...
int main(int argc, char **argv) {
    setlocale(LC_ALL,"");
    ros::init(argc, argv, "left2");
    ros::NodeHandle nh_;
    ros::ServiceServer line_server = nh_.advertiseService("left2", line_server_callback);
    ROS_INFO("视觉巡线初始化");
    if(ros::ok()) ros::spin();
    ROS_INFO("结束");
    return 0;
