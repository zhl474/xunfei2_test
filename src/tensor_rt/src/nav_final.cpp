...
uint16_t room_a_class = -1;//存储三个房间的类型
uint16_t room_b_class = -1;
uint16_t room_c_class = -1;
...
int main(int argc, char** argv)
{
    ...
    if (class_1 == 3 || class_1 == 4 || class_1 == 5)//水果
    {
        target_1 = 3;
        target_2 = 4;
        target_3 = 5;
    }
    if (class_1 == 6 || class_1 == 7 || class_1 == 8)//零食
    {
        target_1 = 6;
        target_2 = 7;
        target_3 = 8;
    ...
    {
        final_class = room_a_class;
        target_room = 1;
        ROS_INFO("目标位于房间A,类型为%d", room_a_class);
    }
    if (room_b_class == target_1 || room_b_class == target_2 || room_b_class == target_3)
    {
        final_class = room_b_class;
        target_room = 2;
        ROS_INFO("目标位于房间B,类型为%d", room_b_class);
    }
    if (room_c_class == target_1 || room_c_class == target_2 || room_c_class == target_3)
    {
        final_class = room_c_class;
        target_room = 3;
        ROS_INFO("目标位于房间B,类型为%d", room_c_class);
    }
