...
#include <geometry_msgs/PointStamped.h>#include <ros/ros.h>
#include <move_base_msgs/MoveBaseAction.h>
#include <actionlib/client/simple_action_client.h>
#include "tensor_rt/Messages.h"
// 新增头文件
...
{
    ROS_INFO("发送观测点 goal_%d", goal_num);
    ac.sendGoal(initial_goal);
    ac.waitForResult();

    if (ac.getState()!= actionlib::SimpleClientGoalState::SUCCEEDED) {
        ROS_ERROR("无法到达观测点 goal_%d", goal_num);
        return -1;
    }
    ROS_INFO("到达观测点 goal_%d. 开始探测目标板.", goal_num);

    // 调用服务进行物体检测
    tensor_rt::Messages srv;//如果调用失败或服务返回的结果为空，则认为检测失败
    ...
    int scan_index = (int)((angle_in_camera - scan_msg->angle_min) / scan_msg->angle_increment);// 将这个角度转换成雷达扫描数据数组中的索引值
    ...
    try {
        tf_buffer.transform(target_point_laser_frame, target_point_map_frame, "map", ros::Duration(1.0));// 使用tf_buffer进行坐标变换，将点从雷达坐标系转换到"map"坐标系，最长等待1秒
    ...
    ac.waitForResult(); // 等待导航完成

    if (ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED) {// 如果成功到达
        ROS_INFO("到达目标板位置 target_%d.", goal_num);
        // 在这里可以再做一次近距离识别确认，如果需要的话
        return detected_class;// 返回本次识别到的物体类别ID
    } else {
...
int main(int argc, char** argv)
{
    ...
    ROS_INFO("Room A -> Class %d, Room B -> Class %d, Room C -> Class %d",
             room_a_class, room_b_class, room_c_class);
    ...
    tf2_ros::Buffer& tf_buffer)
{
    ...
    // 5. 将目标点从雷达坐标系(通常是base_link的子坐标系)转换到map坐标系
    geometry_msgs::PointStamped target_point_laser_frame;// 创建一个带坐标系信息的点
    target_point_laser_frame.header.stamp = ros::Time::now();// 设置时间戳为当前时间
    target_point_laser_frame.header.frame_id = scan_msg->header.frame_id; // 设置坐标系为雷达的坐标系 ("laser_frame")
    target_point_laser_frame.point.x = distance * std::cos(angle_in_camera);
