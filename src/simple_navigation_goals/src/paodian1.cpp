...
amcl_pose_sub_pub::amcl_pose_sub_pub(){
    sub = nh_.subscribe("/amcl_pose", 1, &amcl_pose_sub_pub::amcl_pose_CallBack,this);  
}
void amcl_pose_sub_pub::amcl_pose_CallBack(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg){
    geometry_msgs::Pose msgPointStamped;
    msgPointStamped.position=msg->pose.pose.position;
    msgPointStamped.orientation=msg->pose.pose.orientation;
    ROS_INFO("x = %f, y = %f",msgPointStamped.position.x,msgPointStamped.position.y);
    ROS_INFO("x = %f, y = %f, z = %f, w = %f",msgPointStamped.orientation.x,msgPointStamped.orientation.y,msgPointStamped.orientation.z,msgPointStamped.orientation.w);
}


// struct Point {  
//     double x, y, z,x1,y1,z1,w1;  
// };  
// Point current_position;  

// void feedbackCb(const move_base_msgs::MoveBaseFeedbackConstPtr& feedback) {  
//     current_position.x = feedback->base_position.pose.position.x;  
//     current_position.y = feedback->base_position.pose.position.y;  
//     current_position.z = feedback->base_position.pose.position.z;
//     current_position.x1 = feedback->base_position.pose.orientation.x; 
//     current_position.y1 =feedback->base_position.pose.orientation.y;
//     current_position.z1 =feedback->base_position.pose.orientation.z;
//     current_position.w1=feedback->base_position.pose.orientation.w;
// }
...
int main(int argc, char** argv) {
    ...
    goal[2].target_pose.pose.orientation.z = 0.000;
    goal[2].target_pose.pose.orientation.w = 1.000;  //过坡之后到的点

    goal[3].target_pose.pose.position.x = -1.78;
    goal[3].target_pose.pose.position.y = -0.71;
    ...
    goal[3].target_pose.pose.orientation.z = -0.7002;
    goal[3].target_pose.pose.orientation.w = 0.7140; //读取急救包位置
