...
int cplus_flag = 0;//未找到物资标志位
int laser_flag = 0; //开启雷达扫描标志位 
int laser_process = 0; 
int laser_flag_g = 0; 
int aid_kit_flag = 0; //急救包识别完标志位 
int xun_flag = 0; //开启巡线标志位 
bool arrived_at_board = false;  //板子到达标志位
int edge_direction = 0;        //边缘检测标志位： 0: 无边缘, 1: 右边缘, -1: 左边缘
...
void laserCallback(const boost::shared_ptr<sensor_msgs::LaserScan>& scan) {
    ...
    for (int i = 0; i < num_ranges; ++i) {  
     ...
     // 检测边缘  
   for (int i = 165; i < 202; ++i) {  //110.35度到249.64度
...
void doMsg(const std_msgs::Int8::ConstPtr& msg_p){
    ROS_INFO("doMsg");
    terrorist_count = msg_p->data;   
    if (terrorist_count != 0) {   
...
// 未找到物资回调
void cplus_sub_cb(const std_msgs::Int8::ConstPtr& msg_p){
    cplus_flag = msg_p->data;
}
// 找到物资回调
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber cplus_sub = n.subscribe<std_msgs::Int8>("cplus_topic",10,cplus_sub_cb);   //视觉未找到目标物订阅
