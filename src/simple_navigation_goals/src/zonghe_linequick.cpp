...
void laserCallback(const boost::shared_ptr<sensor_msgs::LaserScan>& scan) {
    ...
    for (int i = 0; i < num_ranges; ++i) {  
     ...
     // 检测边缘  
   for (int i = 160; i < 208; ++i) {  //110.35度到235.64度//225大，最小202
...
for (size_t i = 0; i < board_edges.size() - 1; ++i) {  
    ...
    // 计算板子相对于全局坐标系的角度  
    float global_board_angle_radian = yaw_radian + center_angle_rad - M_PI -M_PI/13;  //远20 近120
...
int main(int argc, char** argv) {
    ...
    goal[9].target_pose.pose.position.x = 0.028;
    goal[9].target_pose.pose.position.y = -0.161;
    ...
    goal[9].target_pose.pose.orientation.z = -0.707;
    goal[9].target_pose.pose.orientation.w = 0.707; //巡线区域
