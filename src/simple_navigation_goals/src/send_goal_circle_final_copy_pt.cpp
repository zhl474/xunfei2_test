...
void roomCallback(const std_msgs::Int16::ConstPtr& a_p)
{
...
int main(int argc, char** argv){
    ...
    ros::Publisher data_request = n.advertise<std_msgs::Int16>("/data_request",1);//publisher
    ros::Subscriber change_room = n.subscribe<std_msgs::Int16>("/room_change",1,roomCallback);
    ...
    goal[5].target_pose.pose.position.y = -3.337;
    ...
    if(room_index==5)//end zhong dian
    {
        ...
        std_msgs::Int16 data_;
        data_.data = 1;
        data_request.publish(data_);
        ros::spinOnce();//用于接收数据处理节点发过来的BCDE数据
        for(int j=0;j<4;j++)
        {
