...
for (size_t i = 0; i < board_edges.size() - 1; ++i) {  
    ...
    // 计算板子相对于全局坐标系的角度  
    float global_board_angle_radian = yaw_radian + center_angle_rad - M_PI -M_PI/18;  //远20 近120
...
int main(int argc, char** argv) {
    ...
    goal[10].target_pose.pose.position.x = -0.015;
    goal[10].target_pose.pose.position.y = -0.121;
