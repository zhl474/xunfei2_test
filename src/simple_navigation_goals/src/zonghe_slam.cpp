...
int awake_flag = 0;
int terrorist_count = 0; // 全局变量，用于存储恐怖分子的数量
...
std_msgs::Int8 xun_detect;//恐怖分子识别启动，为1时启动
std_msgs::Int8 yun_detect;//恐怖分子识别启动，为1时启动
...
void laserCallback(const boost::shared_ptr<sensor_msgs::LaserScan>& scan) {
    ...
    for (int i = 0; i < num_ranges; ++i) {  
     ...
     // 检测边缘  
   for (int i = 160; i < 208; ++i) {  //110.35度到235.64度//225大，中 210 最小202
...
for (size_t i = 0; i < board_edges.size() - 1; ++i) {  
    ...
    // 计算板子相对于全局坐标系的角度  
    float global_board_angle_radian = yaw_radian + center_angle_rad - M_PI -M_PI/13;   
    ...
    float car_x = current_pose.position.x;  
    float car_y = current_pose.position.y;  
    float x1 = car_x + (center_distance - 0.46) * cos(global_board_angle_radian);  
    float y1 = car_y + (center_distance - 0.46) * sin(global_board_angle_radian);  
...
void xun_sub_cb(const std_msgs::Int8::ConstPtr& msg_p){
    ROS_INFO("xun_flag");
    xun_flag_process = msg_p->data;
}
...
int main(int argc, char** argv) {
    ...
    ros::Publisher xun_pub = n.advertise<std_msgs::Int8>("xun_start",10);//开始巡线信息发布
    ros::Publisher yuyin_pub = n.advertise<std_msgs::Int8>("yuyin_start",10);//开始巡线信息发布
    ...
    ros::Subscriber xun_sub = n.subscribe<std_msgs::Int8>("xun_xian",10,xun_sub_cb);   //巡线完成识别订阅
    ...
    goal[5].target_pose.pose.position.y =  -0.055;
    ...
    goal[6].target_pose.pose.position.x = -2.453;
    goal[6].target_pose.pose.position.y = -1.840;
    ...
    lasergoal.target_pose.pose.position.x=-1.752;
    lasergoal.target_pose.pose.position.y=0.004;
    lasergoal.target_pose.pose.position.z=0.000;
    lasergoal.target_pose.pose.orientation.x=0.000;
    lasergoal.target_pose.pose.orientation.y=0.000;
    lasergoal.target_pose.pose.orientation.z=-0.003;
    lasergoal.target_pose.pose.orientation.w=1.000;
    ...
    goal[10].target_pose.pose.position.x = 0.898;
    goal[10].target_pose.pose.position.y = -0.374;
    ...
    goal[10].target_pose.pose.orientation.z = -0.015;
    goal[10].target_pose.pose.orientation.w = 1.000; //进恐怖分子区域

    goal[11].target_pose.pose.position.x = 1.805;
    goal[11].target_pose.pose.position.y = 0.047;
    ...
    goal[11].target_pose.pose.orientation.z = 0.712;
    goal[11].target_pose.pose.orientation.w = 0.702; //终点
    
    

    // goal[12].target_pose.pose.position.x = 1.512;
    // goal[12].target_pose.pose.position.y = -0.442;
    ...
    // goal[12].target_pose.pose.orientation.z = 0.716;
    // goal[12].target_pose.pose.orientation.w = 0.698; //第二个拐角

    // goal[13].target_pose.pose.position.x = 1.498;
    // goal[13].target_pose.pose.position.y = 0.038;
    ...
    // goal[13].target_pose.pose.orientation.z = 0.019;
    // goal[13].target_pose.pose.orientation.w = 1.000; //出恐怖分子区域


    // goal[14].target_pose.pose.position.x = 2.023;
    // goal[14].target_pose.pose.position.y = -0.027;
    // goal[14].target_pose.pose.position.z = 0.000;
    // goal[14].target_pose.pose.orientation.x = 0.000;
    // goal[14].target_pose.pose.orientation.y = 0.000;
    // goal[14].target_pose.pose.orientation.z = 0.704;
    // goal[14].target_pose.pose.orientation.w = 0.710; //到达终点 
    ...
    for (i = 0; i < 12;i++) {
        ...
        // if(i == 10){
        //     xun_detect.data = 1;
        //     xun_pub.publish(xun_detect);
        //     while(xun_flag_process == 0){
        //         ros::spinOnce();
        //     }
        //     //播报
        // }
