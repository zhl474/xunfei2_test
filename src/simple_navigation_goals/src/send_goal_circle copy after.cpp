...
int main(int argc, char** argv){
    ...
    while(room_index != 5) 
    {
            ...
            if(goal_number == 0){
                while(ac.getState() != actionlib::SimpleClientGoalState::SUCCEEDED){
                    ...
                    if( x >= 0.4 && x<= 2.8 && y>= -1.0){                                                  //chufajieduan
                        ...
                        if(x>= 3.6 && y<= -2.5){
                            ...
                            if(y<= -3.2&& success_detect){                  //zai B quyu qie chenggong jiance
                                goal_number += 2;
                                break;
                            }
                            ros::spinOnce();
                ...
                while(ac.getState() != actionlib::SimpleClientGoalState::SUCCEEDED){
                    ...
                    if(y>= -2.8){
                        ...
                        flag_send = 0;
                        if(y>= -1.8 && success_detect){                  //zai B quyu qie chenggong jiance
                            goal_number += 2;
                            goal[goal_number].target_pose.header.frame_id = "map";
                            goal[goal_number].target_pose.header.stamp = ros::Time::now();
                            ac.sendGoal(goal[goal_number]);
        ...
        if(success_detect){
            ...
            room_index = 3;
            goal_number = 5;
            start_detect.data = 10;                                           //到达目标点后，识别位置1
            detect_pub.publish(start_detect);                //发布开启识别
             while(detect_flag == 0){
                ros::spinOnce();
            } 
    ...
    goal[8].target_pose.header.frame_id = "map";
    goal[8].target_pose.header.stamp = ros::Time::now();
    ac.sendGoal(goal[8]);
    flag = 1;
    while(ac.getState() != actionlib::SimpleClientGoalState::SUCCEEDED){
        tf::StampedTransform transform;
    ...
    start_detect.data = 9;                                           //到达目标点后，识别位置1
    detect_pub.publish(start_detect);                //发布开启识别
    ROS_INFO("shut down the camera");
    goal_number = 9;
    room_index = 4;
    while(room_index == 4){
            detect_flag = 0;
            if(goal_number>= 13 && goal_number<=15){
            ...
            else if(goal_number != 10 && goal_number != 11){
                goal[goal_number].target_pose.header.frame_id = "map";
                goal[goal_number].target_pose.header.stamp = ros::Time::now();
                ac.sendGoal(goal[goal_number]);
                while(ac.getState() != actionlib::SimpleClientGoalState::SUCCEEDED);
                ...
                while(ac.getState() != actionlib::SimpleClientGoalState::SUCCEEDED){
                    ...
                    if( y<= -3.2){
                        if(flag){
                            start_detect.data = 3;                                           //到达目标点后，识别位置1
                            detect_pub.publish(start_detect);                //发布开启识别
                            ROS_INFO("successfully send!here in room_index=2");
                            flag = 0;
                            detect_flag = 0;
                        }
                        if(detect_flag){                  //zai B quyu qie chenggong jiance
                            break;
                        }
                        ros::spinOnce();
                    }
            ...
            ros::Duration(0.1).sleep();
            if(goal_number>=12 && goal_number<=15){
                start_detect.data = 6;                                           //到达目标点后，识别
                detect_pub.publish(start_detect);                //发布开启识别
                ROS_INFO("successfully send!!!!MIDDLE"); 
                ros::Duration(0.1).sleep();
            }
            else if(goal_number == 10 && goal_number == 11 && detect_flag == 0){
                 start_detect.data = 9;                                           //到达目标点后，识别
                detect_pub.publish(start_detect);                //发布开启识别
